...
TEMPLATES: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "description": "Nightly retrain of a registered model, then a drift gate that fails loudly when the fresh data no longer matches training distributions.",
        "category": "Data",
        "icon": "refresh-cw",
        "badge": "MLOps",
        "tags": ["model_train", "drift", "psi", "registry", "schedule", "mlops"],
        "docs": "Fully offline demo: the schedule trigger fires nightly (swap in a cron for production), sample training rows are stored into the churn_train dataset, model_train registers 'churn_scorer' in the Model Registry, the stored rows are read back and drift_check PSI-scores them against the fresh reference (no drift expected - point the read at live data to see it flag). on_drift=error fails the run so execution_failed notification rules fire. Activate versions from the Models page.",
        "graph": {
            "nodes": [
                {"id": "tick", "type": "schedule_trigger", "name": "Nightly 02:00", "position": {"x": 0, "y": 0},
                 "parameters": {"mode": "cron", "cron": "0 2 * * *"}},
                {"id": "rows", "type": "set_variable", "name": "Training Rows", "position": {"x": 220, "y": 0},
                 "parameters": {"assignments": {"items": [
                     {"tenure_months": 3, "monthly_spend": 89.5, "support_tickets": 4, "plan": "basic", "churned": 1},
                     {"tenure_months": 34, "monthly_spend": 24.0, "support_tickets": 0, "plan": "basic", "churned": 0},
                     {"tenure_months": 12, "monthly_spend": 55.0, "support_tickets": 1, "plan": "pro", "churned": 0},
                     {"tenure_months": 51, "monthly_spend": 120.0, "support_tickets": 2, "plan": "pro", "churned": 0},
                     {"tenure_months": 5, "monthly_spend": 95.0, "support_tickets": 6, "plan": "pro", "churned": 1},
                     {"tenure_months": 28, "monthly_spend": 33.0, "support_tickets": 1, "plan": "basic", "churned": 0},
                     {"tenure_months": 2, "monthly_spend": 110.0, "support_tickets": 5, "plan": "pro", "churned": 1},
                     {"tenure_months": 40, "monthly_spend": 65.0, "support_tickets": 0, "plan": "pro", "churned": 0},
                     {"tenure_months": 8, "monthly_spend": 70.0, "support_tickets": 3, "plan": "basic", "churned": 1},
                     {"tenure_months": 22, "monthly_spend": 45.0, "support_tickets": 0, "plan": "basic", "churned": 0},
                     {"tenure_months": 15, "monthly_spend": 82.0, "support_tickets": 2, "plan": "pro", "churned": 0},
                     {"tenure_months": 60, "monthly_spend": 150.0, "support_tickets": 1, "plan": "pro", "churned": 0},
                 ]}, "keep_input": False}},
                {"id": "store", "type": "dataset_write", "name": "Store Training Data", "position": {"x": 440, "y": 0},
                 "parameters": {"dataset": "churn_train", "mode": "replace", "create_if_missing": True}},
                {"id": "train", "type": "model_train", "name": "Retrain churn_scorer", "position": {"x": 660, "y": 0},
                 "parameters": {"model": "random_forest_classifier", "task": "classification", "target": "churned",
                                 "model_name": "churn_scorer", "register": True, "cross_validation": 3}},
                {"id": "read", "type": "dataset_read", "name": "Read Stored Rows", "position": {"x": 880, "y": 0},
                 "parameters": {"dataset": "churn_train", "limit": 1000}},
                {"id": "drift", "type": "drift_check", "name": "Drift Gate", "position": {"x": 1100, "y": 0},
                 "parameters": {"model": "churn_scorer", "threshold": 0.25, "on_drift": "error"}},
            ],
            "edges": [
                {"id": "e1", "source": "tick", "target": "rows"},
                {"id": "e2", "source": "rows", "target": "store"},
                {"id": "e3", "source": "store", "target": "train"},
                {"id": "e4", "source": "train", "target": "read"},
                {"id": "e5", "source": "read", "target": "drift"},
            ],
        },
    },
    {
        "id": "nightly-etl-quality-gate",
        "name": "Nightly ETL + Quality Gate",
        "description": "Scheduled ETL chain: cast types, clean nulls, enforce data-quality expectations, upsert into the target dataset and export a CSV artifact - the gate fails the run on bad data.",
        "category": "Data",
        "icon": "database-zap",
        "badge": "ETL",
        "tags": ["etl", "cast", "nulls", "quality", "upsert", "export", "schedule"],
        "docs": "Fully offline demo of the v45/v47 engineering estate: the schedule fires nightly, raw rows are cast to proper types, nulls are zero-filled, data_quality enforces not_null/range/min_rows (on_fail=error hard-fails the pipeline), the clean rows UPSERT into 'dim_customers' by id (idempotent re-runs) and a CSV export artifact lands in Artifacts. Swap the sample rows for your extract nodes in production.",
        "graph": {
            "nodes": [
                {"id": "tick", "type": "schedule_trigger", "name": "Nightly 01:00", "position": {"x": 0, "y": 0},
                 "parameters": {"mode": "cron", "cron": "0 1 * * *"}},
                {"id": "rows", "type": "set_variable", "name": "Raw Extract", "position": {"x": 220, "y": 0},
                 "parameters": {"assignments": {"items": [
                     {"id": "C1", "name": "Acme Corp", "region": "eu", "credit_limit": "5000", "signup": "2024-01-15", "discount": None},
                     {"id": "C2", "name": "Globex", "region": "us", "credit_limit": "12000", "signup": "2023-11-02", "discount": 0.05},
                     {"id": "C3", "name": "Initech", "region": "apac", "credit_limit": "7500", "signup": "2024-03-30", "discount": 0.1},
                     {"id": "C4", "name": "Umbrella", "region": "us", "credit_limit": "9800", "signup": "2024-06-21", "discount": None},
                 ]}, "keep_input": False}},
                {"id": "cast", "type": "cast_columns", "name": "Cast Types", "position": {"x": 440, "y": 0},
                 "parameters": {"casts": [
                     {"column": "credit_limit", "dtype": "number"},
                     {"column": "signup", "dtype": "datetime"},
                     {"column": "discount", "dtype": "number"}
                 ], "on_error": "raise"}},
                {"id": "nulls", "type": "handle_nulls", "name": "Fill Nulls", "position": {"x": 660, "y": 0},
                 "parameters": {"mode": "fill", "fill": "zero", "columns": ["discount"]}},
                {"id": "quality", "type": "data_quality", "name": "Quality Gate", "position": {"x": 880, "y": 0},
                 "parameters": {"checks": [
                     {"check": "not_null", "column": "id"},
                     {"check": "not_null", "column": "credit_limit"},
                     {"check": "range", "column": "credit_limit", "min": 0, "max": 100000},
                     {"check": "min_rows", "min": 4}
                 ], "on_fail": "error"}},
                {"id": "upsert", "type": "dataset_write", "name": "Upsert dim_customers", "position": {"x": 1100, "y": 0},
                 "parameters": {"dataset": "dim_customers", "mode": "upsert", "key_columns": ["id"], "create_if_missing": True}},
                {"id": "export", "type": "dataset_export", "name": "Export CSV", "position": {"x": 1320, "y": 0},
                 "parameters": {"dataset": "dim_customers", "fmt": "csv"}},
            ],
            "edges": [
                {"id": "e1", "source": "tick", "target": "rows"},
                {"id": "e2", "source": "rows", "target": "cast"},
                {"id": "e3", "source": "cast", "target": "nulls"},
                {"id": "e4", "source": "nulls", "target": "quality"},
                {"id": "e5", "source": "quality", "target": "upsert"},
                {"id": "e6", "source": "upsert", "target": "export"},
            ],
        },
    },
]
